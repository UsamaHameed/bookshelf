...
you may find it preferrable to not hit the _actual_ backend during development.
...
mockServer.listen().then(() => {
  // ready to start accepting requests
  // and when you're done, call `mockServer.close()`
})
```

So for this extra credit, let's ditch the `window.fetch.mockImplementation`
stuff and use the mock server instead! You'll want to start listening in a
`beforeAll` and then close it in an `afterAll`.
...
but also things like starting up the mock server and cleaning up the
`queryCache`.

For the setup with our test hooks (like `beforeEach`, `beforeAll`, and
`afterAll`), we can put those in the `src/setupTests.js` file and those will get
run automatically for each test. So in this extra credit move things from our
test file to that file.
